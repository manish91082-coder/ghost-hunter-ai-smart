...
class DataPlane:
    ...
    async def run_heads(self, handler, poll_interval: float = 0.25) -> None:
        ...
        async for block in self.chain.head_poll(poll_interval):
            accepted, replay_start = self.canonical.observe(CanonicalHead(block.number, block.hash, block.parent_hash))
            await handler(block, accepted=accepted, replay_start=replay_start)
